...
[View-only Mode](../user-guide/view-only-mode.md) now provides a dedicated interface for exploring sketches without exposing painting and editing tools. It works both in VR and without a headset, making it useful for exhibitions, demonstrations, shared headsets and dedicated viewer installations. In VR, viewers can move with the Fly or Teleport tools.

The non-VR experience now has a thumbnail grid for browsing local and Icosa Gallery sketches, improved loading and navigation, and touch controls for mobile devices. Dedicated viewer builds start in View-only Mode automatically; regular builds can use the `--ForceViewOnly` command-line switch or matching config setting. This is independent of XR, so it can be combined with `--DisableXrMode` when a flat-screen viewer is required.
