...
Portal widgets are spherical and are saved as part of the sketch. The linked sketch is prepared in the background so moving between connected work feels more immediate.
...
The capture includes the image and depth information needed by downstream tools. You can adjust the capture density and orientation by manipulating the widget, making it easier to concentrate more viewpoints where they are useful.
...
This provides a hardware-independent colocation option for multiplayer sessions. The Multiplayer panel reports the current alignment state and lets participants realign if the shared reference changes.

## Better Steam Frame Support

This release adds a [Steam Frame](../user-guide/platform-and-device-support.md#steam-frame) interaction profile, controller models and controller-specific input support. Links opened by Open Brush can use the Steam overlay browser, including the local forwarding needed to reach Open Brush's web interfaces from the headset.

Together with the Steam Android publishing work in v2.30, these changes make the Steam Frame build behave more like the other fully supported Open Brush targets.
